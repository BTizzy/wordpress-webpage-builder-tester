...
function initScrollAnimations() {
    ...
    // Failsafe: after 1.5s, force-reveal any element still hidden so content is never trapped at opacity:0
    // (anchor jumps, headless screenshot tools, slow IntersectionObserver callbacks, reduced-motion users).
    setTimeout(function () {
    ...
    // Reveal any element that's already in/above the viewport right now (avoids blank sections on first paint).
...
/* === Rachel Edits June 2026 - Motion Features === */

// Highlight expectation items on hover/click for contact page
document.addEventListener('DOMContentLoaded', function() {
    const expectationItems = document.querySelectorAll('.expectation-item');
    expectationItems.forEach(function(item, index) {
        // Click to highlight
        item.addEventListener('click', function() {
            // Remove active from all
            expectationItems.forEach(function(i) { i.classList.remove('expectation-active'); });
            // Add active to clicked
            this.classList.add('expectation-active');
        });
    });
});
